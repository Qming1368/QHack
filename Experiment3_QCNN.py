...
class Quantumnet(nn.Module):
        ...
        def forward(self, input_features):
            ...
            for elem in q_in:
                ...
                tmp = q_out_elem.detach().cpu().numpy()
                #total+=1
                if len(np.unique(tmp)) < 4:
                    eng+=1
                q_out = torch.cat((q_out, q_out_elem))
            return self.post_net(q_out),eng
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                epoch_eng = eng / dataset_sizes[phase]
                if phase == 'val':
                    print('Phase: {} Epoch: {}/{} Loss: {:.4f} Acc: {:.4f} Eng: {:.4f}        '.format('train' if phase == 'train' else 'val  ', epoch + 1, num_epochs, epoch_loss, epoch_acc,epoch_eng))
                    writer.add_scalar('test/acc',epoch_acc,epoch+1)
                    writer.add_scalar('test/eng',epoch_eng,epoch+1)
                    writer.add_scalar('test/loss',epoch_loss,epoch+1)
                    #print('engtanglement nb in val: ',eng)
                if phase == 'train':
                    #print('Phase: {} Epoch: {}/{} Loss: {:.4f} Acc: {:.4f} Eng: {:.4f}        '.format('train' if phase == 'train' else 'val  ', epoch + 1, num_epochs, epoch_loss, epoch_acc,epoch_eng))
                    writer.add_scalar('train/acc',epoch_acc,epoch+1)
                    writer.add_scalar('train/eng',epoch_eng,epoch+1)
                    writer.add_scalar('train/loss',epoch_loss,epoch+1)
                    #print('engtanglement nb in train: ',eng)
        ...
        torch.save(best_model_wts, experiment_dir + 'model_noeng.pt')
...
for i in range(5):
    ...
    best_loss, best_acc = train_model(model_hybrid, criterion, optimizer_hybrid,exp_lr_scheduler, num_epochs=num_epochs)
    test_loss+=best_loss
    test_acc+=best_acc
